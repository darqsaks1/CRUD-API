    ...
    try {
        ...
        req.on('data', data => {
            ...
            if (!obj.username || !obj.age || !obj.hobbies) {
                ...
                res.end(JSON.stringify(`There are not required fileds.Required fileds: ${!obj.username ? 'username' : ''} ${!obj.age ? 'age' : ''} ${!obj.hobbies ? 'hobbies' : ''} `))
            }
            else {
                ...
                res.end(JSON.stringify(`User was saved on "CRUD-API/data/users.json" base.   `))
            }
        })
    } catch (error) {
